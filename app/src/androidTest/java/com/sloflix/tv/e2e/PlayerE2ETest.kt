...
class PlayerE2ETest {
    ...
    @Test
    fun playMovie_showsPlayer_thenBack() {
        composeRule.onFirstPoster().performClick()
        composeRule.waitForTag(TestTags.DetailsRoot, timeoutMs = 45_000).assertIsDisplayed()
        composeRule.waitForTag(TestTags.DetailsPlay, timeoutMs = 30_000).performClick()

        composeRule.waitForTag(TestTags.PlayerRoot, timeoutMs = 180_000).assertIsDisplayed()

        pressBack()
        composeRule.waitForTag(TestTags.DetailsRoot, timeoutMs = 30_000).assertIsDisplayed()
    }
}
